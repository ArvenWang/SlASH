...
- 用户已否决当前 Full Game 的玩家流程与视觉结果，并确认按 V2.1 整体重构。
- 当前交付进度：V2.1 的产品、验收与架构规则已锁定；下一小节把正式 Run 从 23 战 / 4 Boss / 27 技能迁移为 3 章 / 9 战 / 3 Boss / 5 技能家族。
- 当前唯一产品、验收和架构事实源：
...
- 目标玩家流程：Title → 3 章 ×（2 遭遇 + 1 Boss）→ 极简 Victory / Defeat；前 8 战结束后各有一次三选一。
- 正式技能池只保留宽刃、折射、交叉处决、残响斩、杀意；每局最多装备 4 个家族，每个最高 3 级，共 8 次选择。
- 主角、五类敌人、障碍、射弹和三个 Boss 全部迁移为几何体语言；旧 V5R、人形骨骼、刀、武器插槽、血液/断肢和四个旧 Boss 必须退出正式路径。
- 可保留 120Hz 固定步、Seed / Replay、能力输入和通用 Entity 生命周期；旧技能树、旧敌人/Boss 内容与旧表现不再因“已有实现”自动保留。
- 工作方式：由主 Agent 单独精细推进；每个小节只验证直接改动，整体完成后一次完整验证，不重复验证未修改内容。

## Redesign V2.1 下一步

1. 落地 5 技能家族、4 槽、3 级与 8 次确定性奖励，删除旧 27 技能正式池。
2. 将标准局压缩为 3 章 9 战，并迁移 Save / Replay / 自动完整局。
3. 接入三棱体主角、五类敌人、真实垂直物理、带状路径与电光切割。
4. 接入三个新几何 Boss，清理旧双轨路径并完成整体验收。

## Redesign V2.1 已完成

- [x] V2.1 PRD、验收和架构边界锁定 3×3 短局、5/4/3 技能规则、几何角色/敌人/Boss 和残留清理门。
- [x] V2.1 纯规则基线：5 个技能家族 × 3 Rank、4 槽、8 次确定性三选一；3 章 × 3 战、五类敌人和三个 Boss 的精简内容清单。
- [x] Gameplay Arena 扩大为 64m × 40m，普通遭遇 Spawn 覆盖至少 70%；视觉高台基线为 96m × 64m，不再把旧小场地单纯缩远。
- [x] 伤害边界锁定：普通敌人合法冲刺一击必杀；Boss 使用 8/12/16 点生命、开放核心窗口和同源血条，基础/蓄力/大招分别造成 1/2/1 点。
- [x] V2.1 固定步战斗内核：普通敌人无 HP 字段，Boss 独立多段生命；Player/Enemy/Boss/Projectile 均使用显式高度、垂直速度、重力与支撑状态。
- [x] 五类敌人机制接入：追击冲锋、前向几何射击、旋转径向弹、分裂为两个真实子体、起跳/顶点/下落/砸地。
- [x] 冲刺路径内核接入：真实带宽、一次折射、反射距离等级、Stored Path 交叉、单次残响和击杀收招；预览与执行将读取同一 Path Result。
- [x] 三个 Boss 内核接入：显式 `telegraph → active → recovery → vulnerable` 超时链、生命扣除与立即胜利；魔方堡垒空伴随/全死亡伴随均可继续击杀核心。
- [x] 正式入口切换到 V2.1：启动页默认停留，开始后直接进入新 Combat；UI 仅保留生命、能量、Boss 血条、三选一和极简胜负页。
- [x] 模块化几何表现：三棱体主角、五类敌人、三个 Boss、障碍/射弹、160m × 104m 高台、局部边缘反馈和电光/几何碎片 VFX 分别由 Provider 装配。
- [x] 桌面相机完整覆盖真实 64m × 40m Arena；竖屏相机在新 Arena 内平滑跟随，不回退到旧 40m × 25m 区域。
- [x] V2.1 Safe Save 与 Replay：版本/Checksum/状态验证，旧或损坏存档原文保留；真实命令回放 Hash Match。
- [x] V2.1 完整局：真实命令完成 9 战、8 次三选一、3 个多段生命 Boss，Replay Hash Match。
...
## Redesign V2.1 当前状态与问题

- 当前生产代码仍是 V2.0 迁移态：23 战、4 个旧 Boss、27 项奖励、V5R 角色和旧敌人表现尚未符合 V2.1，不得把已有完整局通过视为新版本完成。
- 当前场景仍是第一版结构基线，完成的是 Environment Provider、正式关卡接线与镜头构图，后续仍需继续做视觉精修。
- 当前角色仍走 V5R / Weapon / Corpse 正式装配；这是本轮必须删除的产品残留，不只是“待优化”。
- 当前第二 Boss 的旧逻辑存在伴随单位清除后本体可能永久不可击杀的用户报告；新魔方堡垒必须以唯一核心生命和超时保底解决，并加入回归场景。
- 新正式入口已完成运行时迁移；当前最后未完成项是删除旧 V5R、旧 27 技能、四个旧 Boss、旧 UI/验证/资产和无用依赖，并在清理后做一次最终整体浏览器验收。
- 旧自动化仍有多项把完整树、路线图和详细文案视为成功条件，必须随对应实现小节改写，不能保留为新流程的阻塞门。
- 完整局自动玩家与报告已迁移到 V2；剩余旧非战斗页面验收脚本仍要在相关清理小节逐项改写，但不再作为当前 V2 主流程通过标准。
- 当前编辑由主 Agent 独占，不再启动子代理并行修改。
