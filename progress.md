...
## 2026-08-13 — Redesign V2.1 目标重置

- 用户要求把 V2.0 迁移态继续清理为精简但精致的正式版本，并明确后续由主 Agent 单独精细推进，不再使用子代理。
- 唯一目标已升级到 V2.1：3 章 × 3 战，共 9 场、3 个 Boss、8 次三选一；玩家不看路线、完整树或独立结算。
- 正式奖励池收敛为宽刃、折射、交叉处决、残响斩、杀意五个技能家族；每局最多 4 个家族、每个最高 3 级。
- 角色与内容目标锁定为三棱体主角、五类几何敌人、几何障碍/射弹、三个新几何 Boss、真实垂直物理、持续姿态运动、带状真实击杀范围和电光切割。
- V5R、人形骨骼、刀、武器插槽、血液/断肢、旧 27 技能池、四个旧 Boss 和保护旧体验的验证必须退出正式路径；无引用残留在最终阶段删除。
- 新魔方堡垒以唯一核心生命归零作为胜利事实，并必须覆盖旧第二 Boss 只剩本体后无法击杀的回归场景。
- 本小节只做 PRD、验收、架构边界和进展文档一致性检查；下一小节开始改造 Run 与技能规则。

## 2026-08-13 — V2.1 精简规则与真实大场地基线

- 新增独立纯规则模块：5 个技能家族 × 3 Rank、4 个装备槽、8 次确定性三选一；升级卡使用稳定逐级 ID，禁止重复、越级、第五槽和第四级。
- 1,000 个 Seed 分别连续完成 8 次选择，所有 Offer 均为 3 个不同、合法家族；不存在候选耗尽或假卡。
- 新增 3 章 × 3 战清单，五类普通敌人和棱镜猎犬、魔方堡垒、奇点王冠三个 Boss 各有明确出场边界。
- 根据用户补充，Gameplay Arena 从旧 40m × 25m 扩大到 64m × 40m，面积约 2.6 倍；遭遇 Spawn 包围盒覆盖至少 70%，不能再只拉远镜头。
- 正式场景禁止四条常亮线组成活动方框；视觉高台至少 96m × 64m，边缘只在接近时局部反馈。
- 普通敌人继续一击必杀；Boss 改为 8/12/16 点生命与同源血条，基础冲刺、蓄力冲刺、大招分别造成 1/2/1 点，核心关闭时为 0 点并显示护盾反馈。
- 定向验证：`redesign-skills-run.test.ts` 7 项通过，TypeScript 与 whitespace 通过。本小节尚未声明浏览器运行时已迁移。

## 2026-08-13 — V2.1 固定步战斗内核

- 新增独立 V2.1 `GameState` 与 120Hz 固定步循环；正式目标数据不再依赖旧 Skill Point、Route Graph、V5R 或四 Boss 状态。
- 普通敌人没有生命字段，合法冲刺命中当步死亡；Boss 独立保存 `currentHp / maximumHp / vulnerable`，核心关闭时不扣血。
- 五类敌人机制已真实接入：冲锋、前向几何脉冲、旋转径向弹、分裂为两个子体、真实垂直起跳与砸地。
- Player、Enemy、Boss、Boss Part 与 Projectile 使用显式高度、垂直速度、重力、支撑和落地事实；出界后支撑关闭并下坠。
- 冲刺由统一 Path Result 驱动，包含带宽、一次折射、反射法线/距离、交叉处决、单次残响和杀意收招。
- 三个 Boss 使用统一有界阶段合同；魔方堡垒的空伴随数组与全死亡伴随数组都会当次打开核心并继续结算命中，生命归零立即 `defeated`。
